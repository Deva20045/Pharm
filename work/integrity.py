#!/usr/bin/env python3
"""Integrity check on the built pulse-pharm-complete.html.
...
- per-chapter question counts are sane (>= 5; Pharm chapters are short, 1-14 pp)
Usage: python3 work/integrity.py [pulse-pharm-complete.html]
"""
import json, re, subprocess, sys

HTML = sys.argv[1] if len(sys.argv) > 1 else "pulse-pharm-complete.html"
...
const byCh = {};
QUESTIONS.forEach(q => { const c = q.id.match(/PHARM-C(\\d+)-/)[1]; (byCh[c] = byCh[c] || []).push(q.id); });
out.byChCounts = {};
...
for c, n in o["byChCounts"].items():
    assert n >= 5, f"ch{c} has only {n} questions"
